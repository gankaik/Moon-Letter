...
const tones: Record<keyof typeof game.itemScores, string> = { mooncake:'#e6c17a', golden:'#ffd879', osmanthus:'#e9a934', starlight:'#eaf3ff', lucky:'#f0c46d', cloud:'#8092a0', fiveKernel:'#b99865' }
const labels: Record<keyof typeof game.itemScores, string> = { mooncake:'月', golden:'金', osmanthus:'桂', starlight:'✦', lucky:'福', cloud:'☁', fiveKernel:'五仁' }
...
export function RabbitGame({ onComplete }: Props) {
  ...
  useEffect(() => {
    ...
    const add=()=>{const r=Math.random();const type: keyof typeof game.itemScores = r<.43?'mooncake':r<.57?'osmanthus':r<.68?'golden':r<.76?'starlight':r<.82?'lucky':r<.95?'cloud':'fiveKernel'; items.push({x:.08+Math.random()*.84,y:-24,vy:1.7+Math.random()*1.8,type,spin:Math.random()*6})}
    ...
    const draw=(now:number)=>{const dt=Math.min(32,now-last);last=now; const grad=ctx.createLinearGradient(0,0,0,h);grad.addColorStop(0,'#101f35');grad.addColorStop(1,'#5c7081');ctx.fillStyle=grad;ctx.fillRect(0,0,w,h)
      ...
      for(let i=items.length-1;i>=0;i--){const it=items[i];it.y+=it.vy*dt*.045;const x=it.x*w;ctx.save();ctx.translate(x,it.y);ctx.rotate(Math.sin(now/500+it.spin)*.14);ctx.fillStyle=tones[it.type];ctx.shadowColor=tones[it.type];ctx.shadowBlur=it.type==='starlight'?13:4;ctx.beginPath();ctx.arc(0,0,it.type==='cloud'?17:14,0,Math.PI*2);ctx.fill();ctx.shadowBlur=0;ctx.fillStyle=it.type==='cloud'?'#dce8ef':'#3c3024';ctx.font=it.type==='fiveKernel'?'9px serif':'16px serif';ctx.textAlign='center';ctx.textBaseline='middle';ctx.fillText(labels[it.type],0,1);ctx.restore();if(it.y>h*.78&&Math.abs(x-bunny*w)<42){items.splice(i,1);const val=game.itemScores[it.type];setScore(s=>{const n=s+val;if(n>=game.targetScore)setTimeout(finish,300);return n});if(it.type==='fiveKernel'){setToast('你真的接了五仁月饼？ +1');setTimeout(()=>setToast(''),900)}}else if(it.y>h+25)items.splice(i,1)}
